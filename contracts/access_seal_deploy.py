...
class AccessSeal(gl.Contract):
	...
	@gl.public.view
	def get_case(self,case_id:str)->str:A=case_id;self._require_case(A);B=self._epoch_key(A,self.epochs[A]);return json.dumps({_x:self._address_text(self.buyers[A]),_H:A,_a:int(self.chain_ids[A]),_y:self.contract_addresses[A],_A9:int(self.escrow_amounts[A]),_AA:int(self.evidence_deadlines[A]),'evidenceSealed':self.evidence_sealed[B],'evidenceSealedAt':int(self.evidence_sealed_at[B]),'evidenceSealedBy':self._address_text(self.evidence_sealed_by[B]),_AB:self.flows_hashes[A],_AC:int(self.hard_deadlines[A]),'lifecycle':self.lifecycles[A],_F:int(self.epochs[A]),_AD:int(self.max_unresolved_retries_by_case[A]),_S:self.profile_hashes[A],'reserved':int(self.reserved_by_case[A]),_z:self.salts[A],_R:self.subject_origins[A],'termsHash':self.terms_hashes[A],'vendor':self._address_text(self.vendors[A]),'vendorAccepted':self.vendor_acceptances[A]},sort_keys=_C,separators=(_E,_D))
	@gl.public.view
